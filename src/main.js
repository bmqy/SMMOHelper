...
const SMMOHelper = {
    ...
    // 增加脚本设置项
    addSettingsItem(){
        if(location.href.indexOf('/preferences/customisation')===-1) return false;
        let $wire = document.querySelector('.flex.items-center.justify-between.rounded-lg').parentNode;
        ...
        document.querySelectorAll('button[type=button]').forEach((e) => {
            if(e.innerText === 'Gold'){
                e.addEventListener('click', function () {
                    setTimeout(() => {
                        document.querySelector('.swal2-input').value = 2000
                    }, 0)
                })
            }
        })
